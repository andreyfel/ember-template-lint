...
module.exports = class AttributeIndentation extends Rule {
  ...
  getParamsForBatch(node, isFirstBatch) {
    if (
      (this.config.modifiersFirst && isFirstBatch) ||
      (!this.config.modifiersFirst && !isFirstBatch)
    ) {
      return [node.modifiers, 'element modifier'];
    }
    return [node.attributes, 'htmlAttribute'];
  }
  ...
  validateParams(node) {
    ...
    */
    ...
    if (node.type === 'ElementNode') {
      const [firstBatch, firstBatchType] = this.getParamsForBatch(node, true);
      ...
      const [secondBatch, secondBatchType] = this.getParamsForBatch(node, false);
  ...
  parseConfig(config) {
    ...
    switch (configType) {
      ...
      case 'object': {
        ...
        if ('modifiers-first' in config) {
          result.modifiersFirst = config['modifiers-first'];
        }
    ...
    let errorMessage = createErrorMessage(
      ...
      [
        ...
        '  * { modifiers-first: `boolean` } - `false` : Modifiers must be specified before attributes and arguments',
        ...
        if (this.config.processElements) {
          if (canApplyRule(node, node.type, this.config)) {
            if (node.modifiers.length > 0 || node.attributes.length > 0) {
              let nextLocation = this.validateParams(node);
              if (node.blockParams && node.blockParams.length) {
                nextLocation = {
                  line: nextLocation.line + 1,
